...
const RESTORED: TaskCommandResult = { type: 'ok', changed: true, outcome: { type: 'stopped' } };
const RESTORE_FAILED: TaskCommandResult = {
  type: 'io-error',
  cause: 'time-entry-restore',
...
function collectRows(node: TrackedNode): SessionRow[] {
  ...
  const walk = (
    ...
    for (const entry of snapshot.timeEntries) {
      rows.push({
        key: JSON.stringify([path, entry.relativeLine, entry.originalMarkdown]),
        entry,
        parent: ref,
        ...(title !== undefined && { node: title }),
      });
    }
    for (const [index, subtask] of snapshot.subtasks.entries()) {
      walk(subtask, { type: 'subtask', ref: subtask.ref }, [...path, index], subtask.title);
    }
  };
...
async function removeSession(
  ...
  session.undo.show(
    ...
    async () => ((await actions.restore(recovery)) ? RESTORED : RESTORE_FAILED),
...
function renderRow(session: PopoverSession, row: SessionRow, context: TrackedTimeContext): void {
  ...
  remove.addEventListener('click', (event) => {
    event.stopPropagation();
    runAsyncAction(
      removeSession(session, row, [...session.element.children].indexOf(rowEl)),
      'Could not remove a tracked session',
    );
  });
}
