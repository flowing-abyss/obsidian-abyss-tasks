...
function sourceRangeContains(ranges: readonly SourceRange[], offset: number): boolean {
  return ranges.some((range) => range.from <= offset && offset < range.to);
}

function visibleMarkdownTag(
  body: string,
  ...
  if (sourceRangeContains(excluded, offset) || isEscaped(body, offset)) return null;
  return normalizeTag(match[0]);
}

/** Extracts visible inline body tags using the same Markdown boundaries as lossless tag edits. */
export function extractMarkdownBodyTags(source: string): readonly string[] {
